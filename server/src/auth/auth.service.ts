...
export class AuthService {
  ...
  async register(input: { email: string; password: string; name: string; role?: 'poster'|'helper'|'both' }, device?: string) {
    ...
    const user = { id: this.data.id(), email, name: input.name.trim(), passwordHash: await argon2.hash(input.password), role: input.role || 'both', status: 'active', createdAt: new Date().toISOString() } as const;
  ...
  me(userId: string) { const user = this.data.users.find(item => item.id === userId); if (!user) throw new UnauthorizedException('ACCOUNT_NOT_FOUND'); return { id: user.id, email: user.email, name: user.name, role: user.role, status: user.status }; }
  ...
  private async issue(user: any, device?: string) {
    ...
    return { token: accessToken, accessToken, refreshToken, user: { id: user.id, email: user.email, name: user.name, role: user.role }, state: { tasks: [], notifications: [], messages: [] } };
  }
}
